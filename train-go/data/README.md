...
鉄道の実線路に沿う表示形状は [線路形状データ](rail-shapes.md) を参照。

## 水域データ

島名は水域タイルの地名を使い、収録タイル外の択捉島を補完する。名称・読みは[北海道の島紹介](https://www.pref.hokkaido.lg.jp/sm/hrt/etorofu.html)を参照。東経147.85度・北緯45.1度は島中央付近に文字を置くための表示位置で、測量点ではない。漢字・ひらがなの両表示に対応する。
...
- 新規コースと既存の関東自動生成分は駅間の測地距離を累積した**ゲーム内距離の近似値**。
  標高差や実際の営業キロを保証する値ではない。地図上の線路形状は別途N02から補完するが、ゲーム内距離は維持する。
